...
def init_db():
    ...
    c.execute("CREATE TABLE IF NOT EXISTS prices (key TEXT PRIMARY KEY, value INTEGER)")
...
def stars_menu(message):
    ...
    items = [("stars_100", "100 Stars"), ("stars_500", "500 Stars"), ("stars_1000", "1000 Stars"), ("stars_5000", "5000 Stars"), ("stars_10000", "10000 Stars")]
...
def process_stars(call):
    ...
    conn = sqlite3.connect("smm.db")
    c = conn.cursor()
    c.execute("INSERT INTO orders (user_id, service, amount, price, status) VALUES (?, ?, ?, ?, 'processing')", (user_id, f"Stars {amount}", amount, price))
    order_id = c.lastrowid
    conn.commit()
    conn.close()
    bot.answer_callback_query(call.id, "Обрабатываем...")
    bot.send_message(user_id, f"Заказ на {amount} Stars создан. Ожидайте выдачи.")
...
def admin_set_price(message):
    try:
        ...
        if key.startswith("stars_") or key.startswith("premium_"):
            set_price(key, value)
            bot.reply_to(message, f"Цена {key} обновлена на {value} сум")
        else:
            bot.reply_to(message, "Неизвестный ключ.")
